...
def set_augmentation_space(augmentation_space, num_strengths, custom_augmentation_space_augs=None):
    ...
    if 'xlong' in augmentation_space:
        ...
        ALL_TRANSFORMS = [
            # 从图像中随机剪裁出一块区域，通常将这块区域的像素值设为0（即将这块区域变黑）或设为平均像素值
            # 它有助于模型对图像的局部区域变化保持不变
            cutout,
            # 分别代表左右翻转和上下翻转。它们可以帮助模型处理图像方向的变化
            flip_lr,
            flip_ud,
            # 旋转操作，可以旋转图像的角度。这也有助于模型处理图像方向的变化
            rotate,
            # 在 x 轴和 y 轴方向上的剪切变换，也就是将图像在一个方向上进行倾斜
            shear_x,
            shear_y,
            # 表示在 x 轴和 y 轴方向上的平移变换，即移动图像的位置
            translate_x,
            translate_y,
            # 自动对比度调整方法，可以自动调整图像的亮度和对比度，使图像的视觉效果更佳
            auto_contrast,
            # 直方图均衡化操作，可以改善图像的对比度
            equalize,
            # 颜色反转操作，将图像的颜色反转
            invert,
            # 降低图像颜色深度的操作，可以减少图像的颜色数量
            posterize,
            # crop_bilinear,
            # 反色操作，对图像的亮部进行反色处理。
            solarize,
            # color, contrast, brightness, sharpness: 这四种操作分别对图像的颜色、对比度、亮度和锐度进行调整
            color,
            contrast,
            brightness,
            sharpness,
            # blur 和 smooth: 这两种操作分别对图像进行模糊和平滑处理，可以减少图像的噪声和细节
            blur,
            smooth
        ]
    elif 'ohl' in augmentation_space:
...
set_augmentation_space('full', 31)
...
# # 基本数据增强方法，它使用随机选择的一种转换操作对输入图像进行增强。
# class TrivialAugment:
#     def __call__(self, img, keypoint):
#         # 随机选择一个增强操作
#         op_index = random.randint(0, len(ALL_TRANSFORMS)-1)
#         op = ALL_TRANSFORMS[op_index]
#         # 随机选择一个增强程度
#         level = random.randint(0, PARAMETER_MAX)

#         if op_index == 0: 
#             # 计算原始图像的像素平均值
#             old_image = np.array(img).mean(axis=2)
#             img = op.pil_transformer(1., level)(img)
#             # 计算新图像的像素平均值) 
#             output = np.array(img).mean(axis=2) # (H, W)
#             # 计算新旧图像的差值，得到的结果转换为布尔值（非零元素为True，零元素为False），然后再转换为整数（True转换为1，False转换为0），得到changed_region。
#             # changed_region是一个表示图像变化区域的掩码，然后将这个掩码与关键点相乘，只保留变化区域内的关键点。
#             changed_region = (old_image - output).astype(bool).astype(int) 
#             # 原本表示变化的地方（值为1）现在变为未变化（值为0），原本表示未变化的地方（值为0）现在表示变化（值为1）
#             changed_region = (changed_region - 1) * -1 
#             # 只保留了未变化区域的关键点，变化区域的关键点被置为0
#             keypoint = keypoint * changed_region
#             return img, keypoint # return numpy 
#         elif 1 <= op_index <= 7:
#             img = op.pil_transformer(1., level)(img) 
#             keypoint = Image.fromarray((keypoint * 255).astype(np.uint8))
#             keypoint = op.pil_transformer(1., level)(keypoint)
#             return img, np.array(keypoint)
#         else:
#             # 将该操作应用于输入图像
#             img = op.pil_transformer(1., level)(img)
#             return img, keypoint
...
class TrivialAugment:
    def __call__(self, img, keypoint=None):
        if keypoint is None:
            # 如果没有提供keypoint参数，执行第一种行为
            return self.augment_image(img)
        else:
            # 如果提供了keypoint参数，执行第二种行为
            return self.augment_image_with_keypoint(img, keypoint)
    ...
    def augment_image_with_keypoint(self, img, keypoint):
        op_index = random.randint(0, len(ALL_TRANSFORMS) - 1)
        op = ALL_TRANSFORMS[op_index]
        level = random.randint(0, PARAMETER_MAX)
        ...
        if op_index == 0:
            old_image = np.array(img).mean(axis=2)
            img = op.pil_transformer(1., level)(img)
            output = np.array(img).mean(axis=2)
            changed_region = (old_image - output).astype(bool).astype(int)
            changed_region = (changed_region - 1) * -1
            keypoint = keypoint * changed_region
            return img, keypoint
